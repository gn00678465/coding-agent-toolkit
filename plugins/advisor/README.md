...
- **Safety classifiers re-route the Judgment lane.** Fable 5 and Opus 5 screen for cybersecurity and biology content with deliberately broad safeguards that, by Anthropic's own description, sometimes flag legitimate coding work. A flagged consult is re-run on another model automatically — from Fable, a biology flag lands on Opus 5 and a cybersecurity flag on Opus 4.8 — so a verdict on auth, crypto, or input-validation code can come from a model other than the pinned one. That's expected routing for these domains, not an account problem. The skill treats it as a stand-in and names the model that answered; `claude --safe-mode` tells you whether your own CLAUDE.md, skills, or hooks are what trips it.
